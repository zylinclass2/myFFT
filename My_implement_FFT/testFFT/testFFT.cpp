// testFFT.cpp : Defines the entry point for the console application.
//
...
#define DataAmount ((double)/*1000*/256)
#define SampleRate ((double)/*500*/64)
...
{
int i=0;
complex_t* Data=(complex_t*)malloc(DataAmount*sizeof(complex_t));
double Freq=6;
double Freq2=10;
double Freq3=25;
double Freq4=28.5;
...
{
Data[i].Re=5.3*sin(2*PI*Freq/SampleRate*i)+6.7*sin(2*PI*Freq2/SampleRate*i)+4.3*sin(2*PI*Freq3/SampleRate*i)+9.5*sin(2*PI*Freq4/SampleRate*i)+10;
Data[i].Im=0;
}

if(!fft_any(Data,DataAmount,ForTra))goto BluesteinExecution;

printf("=====FFT正变换结果=====\n");
for(i=0;i<DataAmount;i=i+1)
{
if(i==0)
{
printf("频点%8.3fHz:幅度=%-7.3f 相位=%-7.3f rad\n",(double)i*SampleRate/DataAmount,GetMag(Data[i],DataAmount)/2,GetPha(Data[i]));
continue;
}
printf("频点%8.3fHz:幅度=%-7.3f 相位=%-7.3f rad\n",(double)i*SampleRate/DataAmount,GetMag(Data[i],DataAmount),GetPha(Data[i]));
}
goto ExitCall;
